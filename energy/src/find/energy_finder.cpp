...
int Energy::findFans(const cv::Mat &src) {
    if (src.empty()) {
        if (config.show_info) cout << "empty!" << endl;
        return 0;
    }
    static Mat src_bin;
    src_bin = src.clone();
    if (src.type() == CV_8UC3) {
        cvtColor(src_bin, src_bin,
                 COLOR_BGR2GRAY);  //若读取三通道视频文件，需转换为单通道
    }
    std::vector<vector<Point> > fan_contours;
    FanStruct(src_bin);  //图像膨胀，防止图像断开并更方便寻找
    if (config.show_process) imshow("fan struct", src_bin);
...
int Energy::findArmors(const cv::Mat &src) {
    ...
    std::vector<vector<Point> >
        armor_contours_external;  //用总轮廓减去外轮廓，只保留内轮廓，除去流动条的影响。

    ArmorStruct(src_bin);  //图像膨胀，防止图像断开并更方便寻找
    findContours(src_bin, armor_contours, RETR_LIST, CHAIN_APPROX_NONE);
    if (config.show_process) imshow("armor struct", src_bin);

    findContours(src_bin, armor_contours_external, RETR_EXTERNAL,
                 CHAIN_APPROX_NONE);
    for (int i = 0; i < armor_contours_external.size(); i++)  //去除外轮廓
    {
        ...
        for (int j = 0; j < armor_contours.size(); j++) {
            ...
            if (external_contour_size == all_size) {
                swap(armor_contours[j],
                     armor_contours[armor_contours.size() - 1]);
...
bool Energy::findCenterR(const cv::Mat &src) {
    ...
    if (config.show_process) imshow("R struct", src_bin);
    findContours(src_bin, center_R_contours, RETR_EXTERNAL,
                 CHAIN_APPROX_NONE);
    for (auto &center_R_contour : center_R_contours) {
        ...
        float target_length = target_armor.size.height > target_armor.size.width
                                  ? target_armor.size.height
                                  : target_armor.size.width;
...
bool Energy::findFlowStripFan(const cv::Mat &src) {
    ...
    if (config.show_process) imshow("flow strip fan struct", src_bin);

    findContours(src_bin, flow_strip_fan_contours, RETR_EXTERNAL,
                 CHAIN_APPROX_NONE);
...
bool Energy::findFlowStrip(const cv::Mat &src) {
    ...
    for (auto &candidate_target_armor : candidate_target_armors) {
        ...
        for (int i = 0; i < 4; i++) {
            line(src_bin, vertices[i], vertices[(i + 1) % 4], Scalar(0, 0, 0),
                 20);
        }
    }
    ...
    if (config.show_process) imshow("flow strip struct", src_bin);

    std::vector<vector<Point> > flow_strip_contours;
    findContours(src_bin, flow_strip_contours, RETR_EXTERNAL,
                 CHAIN_APPROX_NONE);
    ...
    for (auto candidate_flow_strip_fan : flow_strip_fans) {
        for (auto &flow_strip_contour : flow_strip_contours) {
            ...
            if (rotatedRectangleIntersection(cur_rect, candidate_flow_strip_fan,
                                             intersection) == 0) {
                continue;
    ...
    if (flow_strips.empty()) {
        ...
        for (auto &candidate_flow_strip_fan : flow_strip_fans) {
            ...
            if (rotatedRectangleIntersection(
                    flow_strip, candidate_flow_strip_fan, intersection) == 0) {
                continue;
            } else if (contourArea(intersection) >
                       energy_part_param_
                           .FLOW_STRIP_CONTOUR_INTERSETION_AREA_MIN) {
        ...
        for (i = 0; i < target_armors.size(); ++i) {
            std::vector<cv::Point2f> intersection;
            if (rotatedRectangleIntersection(target_armors.at(i),
                                             flow_strip_fan, intersection) == 0)
                continue;  //返回0表示没有重合面积
            double cur_contour_area = contourArea(intersection);
            if (cur_contour_area >
                energy_part_param_.TARGET_INTERSETION_CONTOUR_AREA_MIN) {
...
bool Energy::findFlowStripWeak(const cv::Mat &src) {
    ...
    FlowStripStruct(src_bin);  //图像膨胀，防止图像断开并更方便寻找
    if (config.show_process) imshow("weak struct", src_bin);
    ...
    if (flow_strips.empty()) {
        ...
        for (const auto &candidate_flow_strip : flow_strips) {
            for (const auto &candidate_armor : armors) {
                if (pointDistance(candidate_flow_strip.center,
                                  candidate_armor.center) <
                        energy_part_param_.STRIP_ARMOR_DISTANCE_MIN ||
                    pointDistance(candidate_flow_strip.center,
                                  candidate_armor.center) >
                        energy_part_param_.STRIP_ARMOR_DISTANCE_MAX) {
                    continue;
                }
                float angle_armor =
                    candidate_armor.size.width > candidate_armor.size.height
                        ? candidate_armor.angle
                        : candidate_armor.angle - 90;
                float angle_strip = candidate_flow_strip.size.width >
                                            candidate_flow_strip.size.height
                                        ? candidate_flow_strip.angle
                                        : candidate_flow_strip.angle - 90;

                if (abs(angle_armor - angle_strip) < 60 ||
                    abs(angle_armor - angle_strip) > 120) {
                    continue;
                }
...
bool Energy::findCenterROI(const cv::Mat &src) {
    ...
    center_ROI =
        cv::RotatedRect(cv::Point2f(flow_strip.center + p2p * length * 1.7),
