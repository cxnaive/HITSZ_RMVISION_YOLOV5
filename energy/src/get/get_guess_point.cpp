...
bool Energy::guessTarget() {
    ...
    for (const auto &fan : fans) {
        angle = static_cast<float>(
            180 / PI *
            atan2(-1 * (fan.center.y - circle_center_point.y),
    ...
    if (start_guess) {
        ...
        for (i = 1; i < all_fan_angles.size(); ++i) {
            if (abs(min_angle + 72 * i - all_fan_angles.at(i)) >
                energy_part_param_.TWIN_ANGEL_MAX) {
        ...
        if (energy_rotation_direction == CLOCKWISE)
            guess_devide = (guess_devide + 4) % 5;
        else
            guess_devide = (guess_devide + 1) % 5;
    ...
    guess_point.x =
        circle_center_point.x + radius * cos(PI / 180.0 * guess_polar_angle);
    guess_point.y =
        circle_center_point.y - radius * sin(PI / 180.0 * guess_polar_angle);
