...
ILayer* focus(INetworkDefinition* network,
    ...
    ISliceLayer* s1 = network->addSlice(
        input, Dims3{0, 0, 0},
        Dims3{inch, Yolo::INPUT_H / 2, Yolo::INPUT_W / 2}, Dims3{1, 2, 2});
    ...
    auto conv = convBlock(network, weightMap, *cat->getOutput(0), outch, ksize,
                          1, 1, lname + ".conv");
    return conv;
}
...
ILayer* bottleneck(INetworkDefinition* network,
                   ...
                   std::string lname) {
    auto cv1 = convBlock(network, weightMap, input, (int)((float)c2 * e), 1, 1,
                         1, lname + ".cv1");
...
ILayer* bottleneckCSP(INetworkDefinition* network,
    ...
    auto cv1 =
        convBlock(network, weightMap, input, c_, 1, 1, 1, lname + ".cv1");
...
ILayer* C3(INetworkDefinition* network,
    ...
    auto cv1 =
        convBlock(network, weightMap, input, c_, 1, 1, 1, lname + ".cv1");
    auto cv2 =
        convBlock(network, weightMap, input, c_, 1, 1, 1, lname + ".cv2");
    ITensor* y1 = cv1->getOutput(0);
    for (int i = 0; i < n; i++) {
        auto b = bottleneck(network, weightMap, *y1, c_, c_, shortcut, g, 1.0,
                            lname + ".m." + std::to_string(i));
...
IPluginV2Layer* addYoLoLayer(INetworkDefinition* network,
    ...
    ITensor* inputTensors_yolo[] = {det2->getOutput(0), det1->getOutput(0),
                                    det0->getOutput(0)};
